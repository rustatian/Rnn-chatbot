...
class Chatbot:
    ...
    def parseArgs(args):
        ...
        globalArgs.add_argument('--createDataset', action='store_true', help='if present, the program will only '
                                                                             'generate the dataset from the corpus ('
                                                                             'no training/testing)')
        globalArgs.add_argument('--playDataset', type=int, nargs='?', const=10, default=None,  help='if set, the '
                                                                                                    'program  will '
                                                                                                    'randomly play '
                                                                                                    'some samples(can '
                                                                                                    'be use '
                                                                                                    ...
                                                                                                    'action you want '
                                                                                                    'to perform)')
        globalArgs.add_argument('--reset', action='store_true', help='use this if you want to ignore the previous '
                                                                     'model present on the model directory (Warning: '
                                                                     'the model will be destroyed with all the folder '
                                                                     'content)')
        globalArgs.add_argument('--verbose', action='store_true', help='When testing, will plot the outputs at the '
                                                                       'same time they are computed')
        globalArgs.add_argument('--debug', action='store_true', help='run with Tensorflow debug mode. Read TF '
                                                                     'documentation for more details on this.')
        globalArgs.add_argument('--keepAll', action='store_true', help='If this option is set, all saved model will '
                                                                       'be kept (Warning: make sure you have enough '
                                                                       'free disk space or increase saveEvery)')  #
        # TODO: Add an option to delimit the max size
        globalArgs.add_argument('--modelTag', type=str, default=None, help='tag to differentiate which model to '
                                                                           'store/load')
        globalArgs.add_argument('--rootDir', type=str, default=None, help='folder where to look for the models and data')
        globalArgs.add_argument('--watsonMode', action='store_true', help='Inverse the questions and answer when '
                                                                          'training (the network try to guess the '
                                                                          'question)')
        globalArgs.add_argument('--autoEncode', action='store_true', help='Randomly pick the question or the answer '
                                                                          'and use it both as input and output')
        globalArgs.add_argument('--device', type=str, default=None, help='\'gpu\' or \'cpu\' (Warning: make sure you '
                                                                         'have enough free RAM), allow to choose on '
                                                                         'which hardware run the model')
        ...
        datasetArgs.add_argument('--datasetTag', type=str, default='', help='add a tag to the dataset (file where to '
                                                                            'load the vocabulary and the precomputed '
                                                                            'samples, not the original corpus). '
                                                                            'Useful to manage multiple versions. Also '
                                                                            'used to define the file used for the '
                                                                            'lightweight format.')  # The samples are
        #  computed from the corpus if it does not exist already. There are saved in \'data/samples/\ '
        datasetArgs.add_argument('--ratioDataset', type=float, default=1.0, help='ratio of dataset used to avoid '
                                                                                 'using the whole dataset')  # Not
        # implemented, useless ?
        datasetArgs.add_argument('--maxLength', type=int, default=10, help='maximum length of the sentence (for input '
                                                                           'and output), define number of maximum '
                                                                           'step of the RNN')
        datasetArgs.add_argument('--filterVocab', type=int, default=1, help='remove rarelly used words (by default '
                                                                            'words used only once). 0 to keep all '
                                                                            'words.')
        datasetArgs.add_argument('--skipLines', action='store_true', help='Generate training samples by only using '
                                                                          'even conversation lines as questions (and '
                                                                          'odd lines as answer). Useful to train the '
                                                                          'network on a particular person.')
        datasetArgs.add_argument('--vocabularySize', type=int, default=0, help='Limit the number of words in the '
                                                                               'vocabulary (0 for unlimited)')
        ...
        nnArgs.add_argument('--softmaxSamples', type=int, default=0, help='Number of samples in the sampled softmax '
                                                                          'loss function. A value of 0 deactivates '
                                                                          'sampled softmax')
        nnArgs.add_argument('--initEmbeddings', action='store_true', help='if present, the program will initialize '
                                                                          'the embeddings with pre-trained word2vec '
                                                                          'vectors')
        ...
        trainingArgs.add_argument('--saveEvery', type=int, default=2000, help='nb of mini-batch step before creating '
                                                                              'a model checkpoint')
    ...
    def _saveSession(self, sess):
        ...
        with open(model_name, 'w') as f:  # HACK: Simulate the old model existance to avoid rewriting the file parser
            f.write('This file is used internally to check the model existance. Please do not remove.\n')
